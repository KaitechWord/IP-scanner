...
def scan_ssh(ip):
    try:
        nm = nmap.PortScanner()
        res = nm.scan(hosts=ip, arguments="-p 22 -script ssh-hostkey")
...
def scan_ip(ip):
    try:
        nm = nmap.PortScanner()
        res = nm.scan(hosts=str(ip), arguments="-sn")
        if res["scan"][str(ip)]["status"]["state"] == "up":
            return True, scan_ssh(ip)
        else:
            return False, None
...
def scan_ipv6_net(executor, net):
    for ip in net:
        executor.submit(lambda: ("ipv6", *scan_ip(ip)))
